...
export default {
  ...
  async created() {
    ...
    const { data, error } = await supabase
      .from('products')
      .select('*')
      .eq('category', 'icream'); // Lọc sản phẩm theo danh mục 'coffee'
    if (error) {
  ...
  methods: {
    ...
    addToCart(product) {
      ...
      anime({
        ...
        translateX: window.innerWidth - rect.left - 70,
        ...
        complete: () => {
          ...
          if (existingProduct) {
            existingProduct.quantity += 1; // Tăng số lượng sản phẩm nếu đã tồn tại
          } else {
          ...
          // In thông tin giỏ hàng sau khi thêm sản phẩm vào
